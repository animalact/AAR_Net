...
def evalimage(net: Yolact, path: str, save_path: str = None, sess = None, sess_info = None):
    ...
    preds = detect({'loc': torch.from_numpy(pred_onx[0]), 'conf': torch.from_numpy(pred_onx[1]),
                    'mask': torch.from_numpy(pred_onx[2]), 'priors': torch.from_numpy(pred_onx[3]),
...
def evaluate(net: Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        if ':' in args.image:
            ...
            sess = rt.InferenceSession("../../weights/yolact/onnx/yolact.onnx")
            ...
            print(time.time() - prev, "s passed to set model")
            prev = time.time()
            vs = cv2.VideoCapture(args.image)
            while True:
                ret, frame = vs.read()
                print("\n")
                print(time.time()-prev, "s / iter", end="")
                prev = time.time()
                if frame is None:
                    break
                ...
                # my code
                img = frame.copy()
                evalimage(net, img, sess=sess, sess_info=sess_info)
                #cv2.imshow("w", img_numpy)
