...
if __name__ == "__main__":
    select = [6,11,13]
    # select = [2,7,12]
    train_dataset = AARDataset(data_path="./data/", category="dog", anno_path='./data/train', frame_thr=30, skip=15, select=select)
    test_dataset = AARDataset(data_path="./data/", category="dog", anno_path='./data/test', frame_thr=30, skip=1000, select=select, test=True)
