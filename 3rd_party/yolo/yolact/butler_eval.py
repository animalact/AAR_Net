...
def parse_args(argv=None):
    ...
    parser.add_argument('--c', default='results/mask_detections.json', type=str,
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Postprocess'):
        ...
        # t => return classes, scores, boxes, masks
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    preds = net(batch)
    # print(preds[0]['detection']['mask'])
    img_numpy, _masks = prep_display(preds, frame, None, None, undo_transform=False)
...
def evalimages(net:Yolact, input_folder:str, output_folder:str):
    ...
    image_list = sorted(image_list, key=lambda x: int(x.split("timestamp_")[-1].split(".jpg")[0]))
    ...
    for p in image_list:
        ...
        _masks = evalimage(net, path, out_path)     # type: Tensor

        if isinstance(_masks, list):
            _mask = torch.zeros((1, 256, 256))
        else:
            _mask = _masks[0].resize(1,256,256)*255
        if _mask_list is None:
            _mask_list = _mask
        else:
            _mask_list = torch.cat([_mask_list, _mask])
        # print(path + ' -> ' + out_path)
    # print('Done.')
    saveMask(_mask_list)
    # saveMask(_mask_list)
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    def prep_frame(inp, fps_str):
        with torch.no_grad():
            frame, preds = inp
            img_numpy, _masks = prep_display(preds, frame, None, None, undo_transform=False, class_color=True, fps_str=fps_str)
            return img_numpy
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                img_numpy, _masks = prep_display(preds, img, h, w)
            elif args.benchmark:
...
def saveMask(mask: torch.Tensor):
    from skimage import segmentation
    import h5py

    folder = os.path.dirname(args.c)
    vid_name = folder.split("/")[-1]
    h5_name = folder.split("/")[-2]
    filename = f"{h5_name}.h5"

    # if not mask.tolist():
    #     outline_np = np.zeros((1, 256,256))
    # else:
    #     mask = mask[0]
    #     mask_np = mask.cpu().detach().numpy()               # 256, 256, 1   0~1
    #     mask_np = np.reshape(mask_np, (256,256))*255        # 256, 256      0~255
    #     outline_tf = segmentation.find_boundaries(mask_np)
    #     outline_np = outline_tf.astype(float)*255
    #     outline_np = np.reshape(outline_np, (1,256,256))

    with h5py.File(filename, "a") as f:
        # group = f.get(vid_name, None)
        group = f.create_group(vid_name)
        group.create_dataset("data", data=mask.cpu().detach().numpy(), dtype="uint8", compression="gzip")
        # if group is None:
        #     group = f.create_group(vid_name)
        #     group.create_dataset("data", data=mask, maxshape=(None, 256, 256), dtype="uint8",compression="gzip")
        # else:
        #     ds = group['data']
        #     img_id = ds[:].shape[0]
        #
        #     ds.resize(img_id+1, axis=0)
        #     group['data'][img_id:img_id+1] = mask
